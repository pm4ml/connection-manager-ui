...
describe('Test the dfsp egress endpoints thunk actions', () => {
  ...
  it('Should submit the endpoints', async () => {
    ...
    dispatch(changeHubEgressAddress({ address: '192.0.23.23', index: 1 }));
    dispatch(changeHubEgressPort({ port: '90', index: 0, portIndex: 0 }));
    dispatch(changeHubEgressPort({ port: '8080', index: 0, portIndex: 1 }));
    dispatch(changeHubEgressPort({ port: '90', index: 1, portIndex: 0 }));
    dispatch(changeHubEgressPort({ port: '8080', index: 1, portIndex: 1 }));

    await dispatch(submitHubEgressIps());
